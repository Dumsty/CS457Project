...
- **Player Capacity:** 2 Players
- **Game Summary:** A classic turn-based strategy game played on a 3x3 grid. Two players take turns placing their shape (X or O) into an open cell on the grid, trying to be the first to align three of their own shape in a row, column, or diagonal.

### 1.2 Core Game Rules & Win/Draw Conditions
- **Turn Mechanics:** At the start of the game, the server randomly selects Player 1 or Player 2 to go first. Players then alternate turns. Server tracks whose turn it is and only accepts a move from the active player, rejecting out-of-turn moves.
- **Victory Condition:** A player wins when they have three of their own shape in a row, a column, or a diagonal on the 3x3 grid.
- **Draw/Tie Condition:** A draw occurs when all nine cells on the grid are filled and neither player has achieved the victory condition above.
...
- **Router R2 Authoritative DNS:** Configured with `ip dns server` and static host mapping `server.headington.edu` -> `192.168.20.100`.
...
- **Cisco Infrastructure Configuration:** Router R1 DHCP pools (`CLIENT1_POOL`, `CLIENT2_POOL`) and Router R2 authoritative DNS (`ip host server.headington.edu 192.168.20.100`).
